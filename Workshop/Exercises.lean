...
lemma empty_isPolytope : IsPolytope R (∅ : Set X) := by
  use ∅
  simp only [Finset.coe_empty, Convexity.convexHull_empty]
...
lemma intersection_isConvexSet (hP₁ : IsConvexSet R P₁) (hP₂ : IsConvexSet R P₂) :
  ...
  simp at a
  have a1 : ↑w.weights.support ⊆ P₁ := by tauto
  have a2 : ↑w.weights.support ⊆ P₂ := by tauto
  apply hP₁ at a1
  apply hP₂ at a2
  tauto
...
polytope and then use the induction tactic. -/
