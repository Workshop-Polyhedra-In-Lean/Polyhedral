...
variable (k : Type*) {V P : Type*} [DivisionRing k] [StrongRankCondition k]
variable [AddCommGroup V] [Module k V] [AddTorsor V P]
variable (s : Set P)

/-- `cardinalDim k s = 1` only pins down the affine span of `s`, not `s` itself: a proper
segment `s` of a line already spans the whole line without being equal to it. So the correct
statement is an inclusion, not an equality (unlike the `cardinalDim_eq_zero_iff` case, where
`s ⊆ affineSpan k s` and a singleton span forces `s` itself to be that singleton). -/
theorem subset_affineSpan_pair_of_cardinalDim_eq_one
    (h : cardinalDim k s = 1) : ∃ x y : P, x ≠ y ∧ s ⊆ affineSpan k {x, y} := by
  obtain ⟨x, y, hxy, heq⟩ := (AffineSubspace.cardinalDim_eq_one_iff k P (affineSpan k s)).mp h
  exact ⟨x, y, hxy, heq ▸ subset_affineSpan k s⟩

-- TODO: `efinDim_eq_one_iff` has the same bug as the old `cardinalDim_eq_one_iff` (equality of
-- `s` with the span instead of an inclusion) and needs the analogous fix.
