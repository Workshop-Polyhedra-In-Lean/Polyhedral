...
-- lemma hull_invariant_under_scaling (G : Finset V) (μ : V → 𝕜) :
--   (∀ g ∈ G, μ g > 0) ∧  μ.support = G →
--   PointedCone.hull 𝕜 G = PointedCone.hull 𝕜 ((G : Set V).image (fun g => μ g • g)) := by

/-- Scaling the generators of a cone by positive multipliers μ does not change the cone.
    This lemma proves half of this statement. -/
lemma hull_invariant_under_scaling_subset (G1 G2 : Set V)
  (hscaling : ∀ g1 ∈ G1, ∃ g2 ∈ G2, ∃ μ : 𝕜, μ ≥ 0 ∧ g1 = μ • g2) :
  PointedCone.hull 𝕜 G1 ≤ PointedCone.hull 𝕜 G2 := by
    rw [Submodule.span_le, subset_def]
    intro g1 hg1
    obtain ⟨g2, ⟨hg2, ⟨μ, ⟨hμ_nonneg, h_g1_versus_g2⟩⟩⟩⟩ := hscaling g1 hg1
    rw [h_g1_versus_g2]
    have : g2 ∈ hull 𝕜 G2 := mem_span_of_mem hg2
    exact PointedCone.smul_mem (hull 𝕜 G2) hμ_nonneg this
...
theorem IsHPolyhedron.exists_Polytope_plus_Cone_VERSION2 {H : Set A}
  ...
  obtain ⟨D_hom, ⟨G_hom, hG_hom⟩, T, hrepresentation⟩ :=
                     PointedCone.IsHPolyhedral.isPolyhedral this
  -- hrepresentation : H_hom = D_hom ⊔ ↑T
  ...
  set G_hom_pos_normalized := G_hom_pos.image (fun g => g.weight⁻¹ • g) with hG_hom_pos_normalized
    ...
    · intro x hx
      ...
      have hhull : PointedCone.hull 𝕜 X = PointedCone.hull 𝕜 Y := by
        have Y_to_X : ∀ g1 ∈ G_hom_pos_normalized, ∃ g2 ∈ G_hom_pos,
               ∃ μ : 𝕜, μ ≥ 0 ∧ g1 = μ • g2 := by
          intro g1 hg1
          rw [hG_hom_pos_normalized, Finset.mem_image] at hg1
          obtain ⟨g, g_in_G, g_versus_g1⟩ := hg1
          use g
          simp only [g_in_G, true_and]
          use (hom.weight g)⁻¹ -- as μ
          rw [hG_hom_pos, Finset.mem_filter] at g_in_G
          have μ_pos : (hom.weight g)⁻¹ > 0 := inv_pos_of_pos g_in_G.2
          simp only [le_of_lt μ_pos, true_and, ←g_versus_g1]
          rfl
        have X_to_Y : ∀ g1 ∈ G_hom_pos, ∃ g2 ∈ G_hom_pos_normalized,
               ∃ μ : 𝕜, μ ≥ 0 ∧ g1 = μ • g2 := by
          intro g1 hg1_in_G_hom_pos --  g1 ∈ G_hom_pos
          have hg1_in_G_hom_pos_copy := hg1_in_G_hom_pos
          rewrite [hG_hom_pos, Finset.mem_filter] at hg1_in_G_hom_pos_copy
          obtain ⟨g_in_G, g_weight_pos⟩ := hg1_in_G_hom_pos_copy
          change hom.weight g1 > 0 at g_weight_pos
          set g2 := (hom.weight g1)⁻¹ • g1 with hg2_def
          use g2
          rw [hG_hom_pos_normalized, Finset.mem_image]
          constructor
          · use g1
            simp only [hg1_in_G_hom_pos, hg2_def, true_and]
            rfl
          · use hom.weight g1
            constructor
            · exact le_of_lt g_weight_pos
            · simp only [hg2_def, smul_smul, mul_inv_cancel₀ g_weight_pos.ne', one_smul]
      ...
      have z_weight_eq_zero : z_hom.weight = 0 := by
        have hmap_zero : (hull 𝕜 ↑(G_hom_zero : Set V_hom)).map hom.weight = ⊥ := by
          ...
          exact hx.2
        have hz_mem_bot : hom.weight z_hom ∈ (⊥ : PointedCone 𝕜 𝕜) := by
          rw [←hmap_zero, PointedCone.mem_map]
          exact ⟨z_hom, hz_hom, rfl⟩
        rw [mem_bot] at hz_mem_bot
        exact hz_mem_bot
