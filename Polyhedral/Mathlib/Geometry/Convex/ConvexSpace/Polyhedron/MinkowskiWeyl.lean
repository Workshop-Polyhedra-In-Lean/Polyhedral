...
theorem IsHPolyhedron.exists_Polytope_plus_Cone_VERSION2 {H : Set A}
  ...
  · -- The goal is now to show that `H = C +ᵥ P`:
    ...
    · intro x hx
      ...
      obtain ⟨d_hom, hd_hom, s, hs, d_hom_plus_s_eq_x_hom⟩ := mem_sup.mp hC_hom_rep
      rw [← hG_hom] at hd_hom
      ...
      have sum_pos1: ∑ g ∈ G_hom_pos, μ g * g.weight = 1 := by
        have sum_1: ∑ g ∈ G_hom, μ g * g.weight = 1 := by
          have s_weight_zero : s.weight = 0 := T_weight_eq_zero s hs
          ...
          calc
            ...
            _ = d_hom.weight + s.weight     := by rw [s_weight_zero]
            _ = (d_hom + s).weight          := by rw [LinearMap.map_add]
            _ = x_hom.weight                := by rw [d_hom_plus_s_eq_x_hom]
